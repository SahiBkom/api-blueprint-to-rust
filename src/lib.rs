...
pub struct ApibToRs(String);

impl ApibToRs {
    pub fn new(apib_file: String) -> ApibToRs {
        ApibToRs(apib_file)
    }
            ...
            match (event, action.clone()) {
                ...
                (Event::Text(text), Action::Header(header)) => {
                    if text.ends_with(" (object)") {
                        // Attendee
                        debug!("== {}{} ==", header, text);
                        action = Action::Fields(format!(
                            "{}{}",
                            header,
                            text.trim_end_matches(" (object)")
                        ));
                    }
                }
    ...
    pub fn rs_as_string(&self) -> Result<String, Error> {
        self.build()
    }
}
